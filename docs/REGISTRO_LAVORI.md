...
- **[FATTO 4/8, pomeriggio] La gita non era annullata: l'interruttore era rotto.** `getStatoGita()` legge `config_app` con la chiave anonima ed e' fail-closed, ma la policy pubblica consentiva ad anon solo le categorie `branding` ed `editoriale` mentre la chiave della gita e' `feature_flag`: **la lettura non e' mai riuscita**. La funzione ha sempre risposto «annullata», e il 3/8 ha dato il risultato giusto per caso. Corretto con `config_app_chiavi_pubbliche()`, elenco esplicito. In piu' la home, che e' prerenderizzata, aveva congelato «annullata» per un intoppo di rete durante una build: ora `getStatoGitaAlBuild()` fa **fallire la build** invece di pubblicare uno stato non letto. Gita **APERTA fino al 15/8**, i 120 euro di anticipi **non si restituiscono**.
- **[GATE 4/8] Newsletter costruita, mai usata.** Lista con doppio opt-in, campagne via `email_outbox`, registro con vincolo unico, pannello `/newsletter-curatela`. **Nessuna email e' partita.** Restano a Cristian: l'accensione, e l'autorizzazione alla richiesta di consenso ai 39 contatti storici, il cui **testo lo scrive la chat**.
- **[FATTO 4/8, pomeriggio] Chiuse le quattro pendenze che erano ferme in attesa dell'MCP.** Due delle quattro non erano quello che sembravano, ed e' la parte che vale la pena leggere.

  **1. Guida del curatore a Michele Corradini.** Inviata alle 15.43 a `[indirizzo personale del curatore, omesso]` con `info@elbrenz.eu` in copia, identificativo Resend `db628a86`, un solo tentativo. Contenuto: `docs/GUIDA_CURATORE_MUSEO_GG.md` per intero, nove sezioni. Indirizzo verificato contro il database prima di spedire (socio 15, ruolo `curatore_museo_gg`). Rileggendo prima dell'invio sono usciti **due errori di conversione**: la prima versione **perdeva la sezione 1**, e nella seconda la riga «Il titolo» finiva centrata in corsivo come se fosse una firma. Corretti entrambi. E' la ragione per cui una cosa che va a una persona vera si rilegge.

  **2. Eventi di agosto: il file NON e' stato eseguito, ed era la cosa giusta.** Gli eventi erano gia' stati caricati l'1/8, venti righe, tutte gia' **pubblicate**, con chiavi di deduplica diverse da quelle del file: `on conflict do nothing` non avrebbe protetto da niente e l'esecuzione avrebbe creato **nove doppioni** di eventi gia' in pagina. Caricate a mano solo le voci genuinamente mancanti: **quattro** proponibili (L'arte del balocco, Ricordi dal passato al Mulino dalla Torre, Racconti dell'antica corte, Il tempo ritrovato), in stato `proposto`. **Non** caricate le due marcate `non_promuovibile` nel file: usavano la regola vecchia degli organizzatori esclusi, che il direttivo ha poi cambiato in «segnalati, non esclusi», e riclassificarle e' una decisione del direttivo. Stato finale di `eventi_esterni` per la fonte `manuale`: **20 pubblicate, 5 in `proposto`** (le quattro piu' la sfilata di Ossana, vedi sotto).

  Dal confronto sono emerse **due date discordanti**, e le due storie sono diverse:
  - **Ossana Medievale**: NON era un errore. A database 6-9 agosto e' la durata della manifestazione; l'8 agosto del file e' la giornata della sfilata col matrimonio, cioe' un appuntamento DENTRO la festa. «Correggerla» avrebbe accorciato la manifestazione da quattro giorni a uno. La riga resta, e la sfilata e' stata aggiunta come voce sua in `proposto`.
  - **En giro al Casel de Péj**: errore vero. A database diceva «ogni giovedi dal 6 al 27», il pieghevole dice **ogni mercoledi dal 5 al 26**. Corretto su conferma di Cristian, ora 17:00 confermata. **L'errore non si vedeva dai dati**: 6, 13, 20, 27 sono tutti giovedi, quindi la serie sbagliata era perfettamente coerente e nessun controllo automatico l'avrebbe intercettata. Serviva qualcuno con il pieghevole in mano.

  **3. «Alto Adige» nella riga della gita.** Gia' corretto, e la guardia temporanea `sudtirol()` gia' rimossa da `eventi.astro`. Fatta comunque una passata su ogni colonna di testo delle tabelle che finiscono in pagina: restano 29 occorrenze, **tutte da lasciare**, perche' sono nomi ufficiali di enti (Euregio Tirolo-Alto Adige-Trentino, Regione autonoma Trentino-Alto Adige) e citazioni d'archivio. Cambiarle non sarebbe rispetto della regola, sarebbe un errore di fatto.

  **4. Audit lato database.** Vedi la voce qui sotto.
- **[FATTO 4/8] Audit database: un buco vero, chiuso.** `scadi_ordini_creato_vecchi()` e' SECURITY DEFINER e aveva EXECUTE su **PUBLIC**: chiunque con la chiave anonima poteva farla partire via RPC e far scadere in blocco i pagamenti in stato `creato`. Revocato, verificato («permission denied» ad anon), cron intatto. Aggiunto il `search_path` a quattro funzioni. Avvisi passati da **68 a 63**. Cosa resta e perche' non si tocca: le 12 viste SECURITY DEFINER sono le finestre pubbliche del sito, verificate una per una (zero colonne di dati personali, filtri corretti) e passarle a `security_invoker` spegnerebbe il sito; le funzioni chiamabili da anon vivono dentro le policy RLS o sono pubbliche per disegno; quelle che scrivono hanno la guardia di ruolo **dentro**; le 25 tabelle con RLS e nessuna policy sono la chiusura voluta. Dettaglio in `supabase/migrations/20260804230000_audit_db_chiusura.sql`.
- **[FATTO 4/8] Le pagine perdute del vecchio sito: sono 33, non 32.** `docs/legacy_urls.csv` ha ora la colonna **`istituzionale`**, che marca le pagine istituzionali del vecchio sito su criterio di sezione e non di tipo: 53 in tutto, di cui **33 senza corrispondente** (18 sotto `/territorio-e-cultura`, 6 del concorso Os dal Nos, 4 di eventi e manifestazioni, 5 sparse). La differenza rispetto al 32 di prima e' `/territorio-e-cultura/`, la radice della sezione: nel censimento e' tipizzata `pagina` e non `pagina-annidata`, quindi un conteggio fatto sul tipo la lasciava fuori pur essendo **la madre di dodici delle altre**. Numero confermato da Cristian il 4/8. Criterio in `docs/legacy_urls_criterio.md`, dettaglio in `docs/legacy_urls_note.md`. **Lo smistamento resta alla chat**: da `/territorio-e-cultura/` conviene partire.
- **[GATE APERTO 4/8] Mappa storica: lo slot c'e', mancano i fogli.** Il catasto asburgico d'impianto e' pronto a essere acceso e non lo si puo' accendere, ma **non per un problema di licenza**: quella e' **CC BY 4.0**, titolare Provincia Autonoma di Trento, Servizio catasto, e consente ripubblicazione e opere derivate. Il guaio e' che la fonte si e' spostata:
  - il dataset «Mappe storiche d'impianto» su `dati.trentino.it` ha **una sola risorsa**, `catastotn.it/mappeStoriche.html`, che oggi risponde **301 verso `openkat.it`**, portale autenticato del Libro Fondiario: il servizio di scaricamento aperto non e' piu' li';
  - il **GeoServer provinciale** (`siat.provincia.tn.it`) ha **un solo workspace** e 109 livelli, e il catasto storico **non c'e'**: verificato leggendo le capacita' per intero il 4/8.

  **Cosa serve per sbloccare**, ed e' una cosa sola: l'archivio del **comune catastale di Male'**, chiedendolo a `servizio.catasto@provincia.tn.it`, oppure l'indirizzo attuale del servizio di scaricamento se qualcuno lo conosce. Da li' si misura il peso vero di un comune e si decide se estendere alle quattro valli.

  **Cosa e' gia' fatto e non va rifatto:** `src/lib/catastoStorico.ts` tiene le coperture (oggi vuote), i confini, gli zoom utili e l'attribuzione; il pannello sulla mappa spiega l'assenza invece di lasciare un riquadro bianco, e allo stesso posto comparira' il cursore della trasparenza; il selettore di sfondo e' gia' a tre voci e la quarta si accende da sola con i dati. **Accendere il livello e' UNA VOCE** nell'array `COPERTURE`.

  **Due cose in sospeso da non perdere:** (1) **GDAL non e' installato** sulla macchina, quindi la catena per generare i riquadri va messa in piedi prima; (2) la **datazione**: il brief diceva 1856-1861, ma il dataset della Provincia data le proprie mappe **1817-1855**, e sulla pagina va la datazione della fonte che si mostra. Il testo divulgativo lo scrive la chat.
- **[GATE APERTO 4/8] Informativa privacy: residenza e telefono.** Le quindici colonne anagrafiche esistono, il pannello del segretario le usa, il libro soci le stampa. Ma nel **modulo pubblico i campi non compaiono**, perché `/privacy` dichiara oggi che il tesseramento non raccoglie il telefono. Chiederlo prima di aver riscritto l'informativa la renderebbe falsa, e un'informativa falsa è una violazione mentre una scarna è solo una limitazione. Interruttore: `src/lib/anagrafica.ts`, `ANAGRAFICA_PUBBLICA_LIVE = false`. Per accenderlo, nell'ordine: la chat riscrive `/privacy` con finalità e base giuridica di residenza e telefono → Cristian approva e va online → la chat conferma le etichette → flag a `true`, build, deploy.
