...
- **[FATTO 4/8, pomeriggio] Chiuse le quattro pendenze ferme sull'MCP.** (1) **Guida a Michele Corradini** inviata alle 15.43, `mic.trento@virgilio.it` con `info@elbrenz.eu` in copia, identificativo Resend `db628a86`. (2) **Eventi di agosto**: il file `docs/selezione_eventi_agosto_2026.sql` **non e' stato eseguito**, ed era giusto cosi': gli eventi erano gia' stati caricati l'1/8 e sono gia' pubblicati, con chiavi di deduplica diverse. Eseguirlo avrebbe creato **nove doppioni** sul sito. Caricate a mano solo le **quattro voci genuinamente mancanti**, in stato `proposto`. (3) **«Alto Adige» nella riga della gita**: gia' corretto, e la guardia `sudtirol()` gia' rimossa. Le occorrenze rimaste sono nomi ufficiali di enti (Euregio Tirolo-Alto Adige-Trentino, Regione autonoma Trentino-Alto Adige) e citazioni d'archivio: **non si toccano**. (4) **Audit lato database chiuso**: vedi la voce sotto.
